...
Simulator ──webhook──▶ /webhook (main.py) ──▶ db_hook.py ──▶ events (raw, always) ──▶ parking.py ──▶ parking_spots, parking_sessions, events
...
`SEQUENCE_GAP`, `SEQUENCE_OUT_OF_ORDER`, `PAYMENT_REJECTED`, `CHARGE_FAILED`, `SESSION_RECOVERED`,
`PENALTY`, `CO_ALERT`.
...
> **Not used by the live flow yet.** `main.py` picks spots itself (random free spot from
> `list-parking-spots`, `asyncio.Lock`). `allocate_spot()` below is ready and tested if the team
> wants the database to choose spots instead (safe across several backend processes, no `list-*` call per car).
...
`POST /webhook` is in [`main.py`](../backend/fastapi_project/main.py) (Tee's car flow). It also hands every
webhook to [`db_hook.py`](../backend/fastapi_project/db_hook.py), whose background worker runs
[`webhook_handlers.py`](../backend/app/services/webhook_handlers.py) `receive()` (checks + raw copy) and `process()`
(update the tables). The database side never sends commands to the simulator.
...
| `EventClass` | Car flow (`main.py`) | Database (`db_hook.py`) |
| --- | --- | --- |
| `car_spot_action` EntrySpot/CarIn | pick a free spot → open gate → `goto` → close after 5 s (full → `leavepark`) | raw copy only |
| `car_spot_action` Park/CarIn → Park/CarOut | – | spot `OCCUPIED` + session `PARKED` → spot `FREE`, session `EXITING` |
| `car_spot_action` ExitSpot/CarIn | charge once → Gate B: open → `leavepark` → close after 3 s | raw copy only |
| `car_spot_action` ExitSpot/CarOut | – | session `COMPLETED` |
| `payment_made` | – | raw copy only (validating it needs the charge, which `main.py` keeps in memory) |
...
A car's session starts when it is detected in its spot (the database doesn't see the spot choice),
so `entry_time` is the time it parked.
...
1. Which gate is which: the level has `gateA`, `gateB`, `gateC`. `main.py` opens a gate named like the entry spot (`ENTRY1`) and `GateB`. Tested: opening `ENTRY1` does nothing (the simulator still answers 201), `GateB` and `gateA` do open. **The entry gate should probably be `gateA`.**
